...
<template>
  ...
  >
    ...
    <div class="px-3 py-2 border-b border-gray-200 dark:border-dark-border flex items-center justify-between">
      <h3 class="text-xs font-semibold text-gray-900 dark:text-white">Category Editor</h3>
      <div class="flex items-center gap-4">
        <label class="flex items-center gap-1.5 cursor-pointer">
          <input
            ...
            v-model="mode"
            value="select"
            class="w-3 h-3 text-primary-500 border-gray-300 dark:border-gray-600 focus:ring-primary-500"
          />
          <span class="text-xs text-gray-700 dark:text-gray-300">Select</span>
        </label>
        <label class="flex items-center gap-1.5 cursor-pointer">
          <input
            type="radio"
            ...
            class="w-3 h-3 text-primary-500 border-gray-300 dark:border-gray-600 focus:ring-primary-500"
          />
          <span class="text-xs text-gray-700 dark:text-gray-300">Direct</span>
        </label>
      </div>
    </div>
          ...
          <div class="flex-1 min-w-0 space-y-1">
            ...
            <div class="relative process-dropdown">
              <button
                ...
                class="w-full flex items-center justify-between px-2 py-1.5 text-xs bg-gray-50 dark:bg-dark-border border border-gray-200 dark:border-gray-600 rounded-md text-left hover:border-primary-300 dark:hover:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
              >
                <span class="truncate" :class="selectedProcess ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'">
              ...
              <div
                ...
                class="absolute z-50 w-48 mt-1 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-md shadow-lg max-h-48 overflow-hidden"
              >
                <!-- Search Input -->
                <div class="p-2 border-b border-gray-100 dark:border-dark-border/50">
                  <input
                    ...
                    type="text"
                    placeholder="Search..."
                    class="w-full px-2 py-1 text-xs border border-gray-200 dark:border-gray-600 rounded bg-gray-50 dark:bg-dark-border text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary-500"
                    @click.stop
                  />
                </div>
                <!-- Options -->
                <div class="max-h-32 overflow-y-auto">
                  <button
                    ...
                    class="w-full px-3 py-1.5 text-xs text-left hover:bg-gray-100 dark:hover:bg-dark-border transition-colors"
                  ...
                  <div v-if="filteredProcesses.length === 0" class="px-3 py-2 text-xs text-gray-400 dark:text-gray-500">
                    No results
                  </div>
                </div>
              </div>
          ...
          <div class="flex-1 min-w-0 space-y-1">
            ...
            <div class="relative model-dropdown">
              <button
                ...
                class="w-full flex items-center justify-between px-2 py-1.5 text-xs bg-gray-50 dark:bg-dark-border border border-gray-200 dark:border-gray-600 rounded-md text-left hover:border-primary-300 dark:hover:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <span class="truncate" :class="selectedModel ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'">
                  <template v-if="loadingModels">Loading...</template>
                  <template v-else>{{ selectedModel || 'Select...' }}</template>
                </span>
                <svg v-if="loadingModels" class="w-3 h-3 text-gray-400 animate-spin flex-shrink-0 ml-1" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <svg v-else class="w-3 h-3 text-gray-400 flex-shrink-0 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              ...
              >
                ...
                <div class="max-h-32 overflow-y-auto">
                  ...
                  <div v-if="filteredModels.length === 0" class="px-3 py-2 text-xs text-gray-400 dark:text-gray-500">
                    {{ availableModels.length === 0 ? 'No models' : 'No results' }}
                  </div>
                </div>
              </div>
            </div>
      ...
      <template v-else>
        <div class="space-y-1">
          ...
          <div class="flex items-center">
            <span class="px-2 py-1.5 text-xs bg-gray-100 dark:bg-gray-700 border border-r-0 border-gray-200 dark:border-gray-600 rounded-l-md text-gray-500 dark:text-gray-400">
              {{ prefix }}
            </span>
            <input
              ref="directInputRef"
              ...
              class="flex-1 px-2 py-1.5 text-xs bg-gray-50 dark:bg-dark-border border border-gray-200 dark:border-gray-600 rounded-r-md text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-primary-500"
            />
          </div>
        </div>
      </template>
    ...
    <div class="px-3 py-2 border-t border-gray-200 dark:border-dark-border flex items-center justify-between gap-3 mt-2">
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        <button
          @click="handleCancel"
          type="button"
          class="px-2.5 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-dark-border hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors"
        >
          Cancel
        </button>
        <button
          @click="handleApply"
          type="button"
          class="px-2.5 py-1 text-xs font-medium text-white bg-primary-500 hover:bg-primary-600 rounded transition-colors"
        >
          Apply
        </button>
      </div>
