...
const processDistForChart = computed(() => {
  return (props.drilldown?.processDistribution || []).map(p => ({
    trigger_by: p.process,
    total: p.total
  }))
})
</script>
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-dark-bg rounded-lg mt-2 border border-gray-200 dark:border-dark-border">
    ...
    <div>
      <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">공정 분포</h4>
      <RecoveryTriggerChart
        v-if="processDistForChart.length"
        :data="processDistForChart"
      />
      <div v-else class="flex items-center justify-center text-gray-400 dark:text-gray-500 text-sm py-8">
        데이터가 없습니다
      </div>
