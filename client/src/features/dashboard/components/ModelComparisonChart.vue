...
const option = computed(() => {
  ...
  const categories = props.data.map(d => d.model)
  ...
  return {
    ...
    ...(needsZoom.value ? {
      dataZoom: [{
        ...
        brushSelect: false, handleSize: '60%',
    ...
    xAxis: {
      ...
      axisLabel: { color: dark ? '#d1d5db' : '#374151', fontSize: 11, rotate: categories.length > 8 ? 35 : 0, overflow: 'truncate', width: 70 },
      axisLine: { lineStyle: { color: dark ? '#374151' : '#e5e7eb' } },
      axisTick: { show: false },
      axisPointer: { type: 'shadow', label: { show: true, backgroundColor: dark ? '#374151' : '#111827', color: '#fff', fontSize: 12, padding: [4, 8] } }
    },
    yAxis: {
      type: 'value', max: 100,
    ...
    animationEasing: 'elasticOut', animationDuration: 800,
    series: RATE_STATUSES.map((status, idx) => ({
      name: status, type: 'bar', stack: 'rate', data: seriesData[status],
      itemStyle: { color: statusColors[status], borderRadius: idx === RATE_STATUSES.length - 1 ? [3, 3, 0, 0] : [0, 0, 0, 0] },
...
function buildStackedOption(dark, categories) {
  ...
  return {
    ...
    legend: { top: 0, textStyle: { color: dark ? '#9ca3af' : '#6b7280' } },
    grid: { left: 10, right: 20, top: 32, bottom: needsZoom.value ? 50 : 10, containLabel: true },
    ...(needsZoom.value ? {
      dataZoom: [{
        type: 'slider', xAxisIndex: 0, bottom: 5, height: 16,
        startValue: 0, endValue: MAX_VISIBLE - 1,
    ...
    series: allStatuses.map((status, idx) => ({
      name: status, type: 'bar', stack: 'total', data: seriesData[status],
      itemStyle: { color: statusColors[status], borderRadius: idx === allStatuses.length - 1 ? [3, 3, 0, 0] : [0, 0, 0, 0] },
      emphasis: { itemStyle: { shadowBlur: 6, shadowColor: 'rgba(0,0,0,0.15)' } },
      barMaxWidth: 50
    }))
  }
