...
const option = computed(() => {
  ...
  const okData = props.data.map(d => d.okCount)
  const warnData = props.data.map(d => d.warnCount)
  const shutdownData = props.data.map(d => d.shutdownCount)
  const stoppedData = props.data.map(d => d.stoppedCount || 0)
  const neverStartedData = props.data.map(d =>
    d.agentCount - d.okCount - d.warnCount - d.shutdownCount - (d.stoppedCount || 0)
  )
    ...
    series: [
      {
        ...
        stack: 'total',
        data: okData,
        itemStyle: { color: dark ? '#4ade80' : '#22c55e', borderRadius: [0, 0, 0, 0] },
      ...
      },
      {
        ...
        stack: 'total',
        data: warnData,
        itemStyle: { color: dark ? '#f97316' : '#f59e0b', borderRadius: [0, 0, 0, 0] },
        emphasis: { itemStyle: { shadowBlur: 6, shadowColor: 'rgba(0,0,0,0.15)' } },
        barMaxWidth: 100
      },
      {
        ...
        stack: 'total',
        data: shutdownData,
        itemStyle: { color: dark ? '#ef4444' : '#dc2626', borderRadius: [0, 0, 0, 0] },
        emphasis: { itemStyle: { shadowBlur: 6, shadowColor: 'rgba(0,0,0,0.15)' } },
        barMaxWidth: 100
      },
      {
        ...
        stack: 'total',
        data: stoppedData,
        itemStyle: { color: dark ? '#6b7280' : '#9ca3af', borderRadius: [0, 0, 0, 0] },
        emphasis: { itemStyle: { shadowBlur: 6, shadowColor: 'rgba(0,0,0,0.15)' } },
        barMaxWidth: 100
      },
      {
        ...
        itemStyle: { color: dark ? '#374151' : '#d1d5db', borderRadius: [4, 4, 0, 0] },
        emphasis: { itemStyle: { shadowBlur: 6, shadowColor: 'rgba(0,0,0,0.15)' } },
        barMaxWidth: 100
      }
    ]
