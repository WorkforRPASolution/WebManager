...
const totalOkCount = computed(() => props.data.reduce((sum, r) => sum + r.okCount, 0))
const totalWarnCount = computed(() => props.data.reduce((sum, r) => sum + r.warnCount, 0))
const totalShutdownCount = computed(() => props.data.reduce((sum, r) => sum + r.shutdownCount, 0))
const totalStoppedCount = computed(() => props.data.reduce((sum, r) => sum + (r.stoppedCount || 0), 0))
const totalNeverStarted = computed(() =>
  totalAgentCount.value - totalOkCount.value - totalWarnCount.value - totalShutdownCount.value - totalStoppedCount.value
)

function neverStarted(row) {
  return row.agentCount - row.okCount - row.warnCount - row.shutdownCount - (row.stoppedCount || 0)
}

function rateText(agent, ok, warn) {
  if (agent === 0) return '—'
  return (((ok + warn) / agent) * 100).toFixed(0) + '%'
}

function rateClass(agent, ok, warn) {
  const active = ok + warn
  if (agent === 0) return 'bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400'
  if (active === agent) return 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
  if (active > 0) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'
...
<template>
  <div>
    ...
    >
      Redis 미연결 상태입니다. 상태 정보가 정확하지 않을 수 있습니다.
    </div>
    ...
    <table v-else-if="data.length > 0" class="w-full text-sm">
      <thead>
        <tr class="border-b border-gray-200 dark:border-dark-border">
          ...
          <th class="text-right py-3 px-4 font-semibold text-gray-600 dark:text-gray-300">OK</th>
          <th class="text-right py-3 px-4 font-semibold text-gray-600 dark:text-gray-300">WARN</th>
          <th class="text-right py-3 px-4 font-semibold text-gray-600 dark:text-gray-300">SHUTDOWN</th>
      ...
      <tbody>
        ...
        >
          ...
          <td class="py-3 px-4 text-right">
            <span :class="row.okCount > 0 ? 'text-green-600 dark:text-green-400' : 'text-gray-400'">
              {{ row.okCount }}
            </span>
          </td>
          <td class="py-3 px-4 text-right">
            <span :class="row.warnCount > 0 ? 'text-orange-500 dark:text-orange-400' : 'text-gray-400'">
              {{ row.warnCount }}
            </span>
          </td>
          <td class="py-3 px-4 text-right">
            <span :class="row.shutdownCount > 0 ? 'text-red-500 dark:text-red-400' : 'text-gray-400'">
              {{ row.shutdownCount }}
            </span>
          </td>
          <td class="py-3 px-4 text-right">
            <span :class="(row.stoppedCount || 0) > 0 ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400'">
              {{ row.stoppedCount || 0 }}
            </span>
          </td>
          <td class="py-3 px-4 text-right">
          ...
          <td class="py-3 px-4 text-right">
            <span
              ...
              :class="rateClass(row.agentCount, row.okCount, row.warnCount)"
            >
              {{ rateText(row.agentCount, row.okCount, row.warnCount) }}
            </span>
          </td>
        </tr>
      </tbody>
      <tfoot>
        <tr class="border-t-2 border-gray-300 dark:border-dark-border font-semibold">
          ...
          <td class="py-3 px-4 text-right text-green-600 dark:text-green-400">{{ totalOkCount }}</td>
          <td class="py-3 px-4 text-right text-orange-500 dark:text-orange-400">{{ totalWarnCount }}</td>
          <td class="py-3 px-4 text-right text-red-500 dark:text-red-400">{{ totalShutdownCount }}</td>
          <td class="py-3 px-4 text-right text-gray-600 dark:text-gray-400">{{ totalStoppedCount }}</td>
          ...
          <td class="py-3 px-4 text-right">
            <span class="inline-block min-w-[3rem] text-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
              {{ rateText(totalAgentCount, totalOkCount, totalWarnCount) }}
            </span>
          </td>
        </tr>
      </tfoot>
