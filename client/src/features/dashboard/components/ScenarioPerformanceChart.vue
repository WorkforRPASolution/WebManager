...
const option = computed(() => {
  ...
  const filledValues = items.map(d => d.performanceFilled || 0)
  const unfilledValues = items.map(d => (d.total || 0) - (d.performanceFilled || 0))
  ...
  return {
    tooltip: {
      ...
      formatter: (params) => {
        ...
        if (!item) return name
        const filled = item.performanceFilled || 0
        const total = item.total || 0
        return `<b>${name}</b><br/>` +
          `입력: <b>${filled}</b><br/>` +
          `미입력: <b>${total - filled}</b><br/>` +
          `전체: <b>${total}</b><br/>` +
          `입력률: <b>${(item.performanceRate || 0).toFixed(1)}%</b>`
      }
    },
    legend: {
