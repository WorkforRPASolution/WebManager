...
const totalUsers = computed(() => (props.data || []).reduce((s, d) => s + d.activeUsers, 0))
...
const option = computed(() => {
  ...
  const pieData = items.map((d, i) => ({
    name: d.process,
    value: d.activeUsers,
    itemStyle: { color: colors[i % colors.length] }
  }))

  const totalText = totalUsers.value.toLocaleString()
  ...
  return {
    ...
    series: [{
      ...
      label: {
        ...
        rich: {
          total: { fontSize: 28, fontWeight: 'bold', color: dark ? '#fff' : '#111827', lineHeight: 36 },
          label: { fontSize: 11, color: dark ? '#9ca3af' : '#6b7280', lineHeight: 16 }
        }
      },
      emphasis: {
        label: { show: true },
