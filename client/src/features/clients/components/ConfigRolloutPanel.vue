<template>
  <div class="w-80 border-l border-gray-200 dark:border-dark-border bg-gray-50 dark:bg-dark-bg flex flex-col overflow-hidden shrink-0">
    ...
    <div v-if="activeFileHasChanges" class="px-4 py-2.5 bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 text-xs flex items-start gap-2">
      <svg class="w-4 h-4 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
      </svg>
      <span>저장되지 않은 변경사항이 있습니다. 배포는 현재 편집 중인 내용이 아닌 <strong>마지막 저장된 버전</strong>으로 실행됩니다.</span>
    </div>
...
const props = defineProps({
  ...
  activeFileHasChanges: { type: Boolean, default: false }
})

const emit = defineEmits(['close', 'deploy-complete'])
    ...
    await fetchSSEStream(
      ...
      },
      {
        ...
        onDone: (data) => {
          ...
          // Notify parent to refresh target clients' cached configs
          const successEqpIds = clientResults.value
            .filter(r => r.status === 'success')
            .map(r => r.eqpId)
          if (successEqpIds.length > 0) {
            emit('deploy-complete', successEqpIds)
          }
        }
      }
    )
