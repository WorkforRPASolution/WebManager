      ...
      >
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-dark-border bg-gray-50 dark:bg-dark-bg shrink-0 select-none" :class="{ 'cursor-move': !isMaximized }" @mousedown="startDrag" @dblclick="toggleMaximize">
          <div class="flex items-center gap-2">
            ...
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Config File Settings</h3>
          </div>
          <div class="flex items-center gap-2">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-dark-border">
                ...
                <tr v-if="items.length === 0">
                  <td colspan="3" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                    No config files. Click "Add" to create one.
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 dark:border-dark-border">
          ...
          <div class="flex gap-3">
            ...
            <button
              ...
              class="flex items-center gap-2 px-4 py-2 text-sm bg-purple-500 hover:bg-purple-600 text-white rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
async function loadData() {
  ...
  try {
    const response = await clientConfigApi.getSettingsDocument(props.agentGroup)
    const doc = response.data
    items.value = (doc.configFiles || []).map(f => ({
      _key: `k_${keyCounter++}`,
      fileId: f.fileId,
      name: f.name,
      path: f.path,
...
async function handleSave() {
  ...
  saveError.value = null
  try {
    const configFiles = items.value.map(item => ({
      fileId: item.fileId,
      name: item.name.trim(),
      path: item.path.trim()
    }))
    await clientConfigApi.saveSettingsDocument(props.agentGroup, configFiles)
    changed.value = false
    emit('saved')
  } catch (error) {
