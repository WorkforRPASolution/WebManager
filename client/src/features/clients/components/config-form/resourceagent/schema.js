...
export const RESOURCEAGENT_SCHEMA = {
  fields: {
    // ── 전송 방식 ──
    SenderType: {
      type: 'select', label: '전송 방식', default: 'kafka',
      description: '메트릭 전송 방식을 선택합니다. kafka: 직접 전송, kafkarest: KafkaRest Proxy, file: 로컬 파일',
      options: [
        { value: 'kafkarest', label: 'KafkaRest Proxy' },
        { value: 'kafka', label: 'Kafka 직접 전송' },
    ...
    // ── 파일 출력 ──
    'File.FilePath': {
      type: 'text', label: '파일 경로', default: 'log/ResourceAgent/metrics.jsonl',
    ...
    'File.Console': {
      type: 'boolean', label: '콘솔 출력', default: true,
      description: '콘솔(stdout)에도 메트릭을 출력할지 여부입니다.'
    },
    'File.Pretty': {
    ...
    'File.Format': {
      type: 'select', label: '출력 포맷', default: '',
      description: '파일 출력 포맷입니다. 빈 값은 legacy 동작과 동일합니다.',
      options: [
        { value: '', label: '기본값 (legacy)' },
        { value: 'json', label: 'JSON' },
        { value: 'legacy', label: 'Legacy (EARS 호환)' }
      ]
    },

    // ── Kafka 설정 ──
    'Kafka.Brokers': {
      type: 'array', label: 'Kafka 브로커', default: ['localhost:9092'],
      description: 'Kafka 브로커 주소 목록입니다. host:port 형식으로 입력합니다.'
    },
    'Kafka.Compression': {
    ...
    'Kafka.MaxRetries': {
      type: 'number', label: '최대 재시도', default: 3,
      description: '전송 실패 시 최대 재시도 횟수입니다.'
    },
    'Kafka.RetryBackoff': {
      type: 'go-duration', label: '재시도 대기', default: '100ms',
      description: '재시도 간 대기 시간입니다. Go duration 형식 (예: 100ms, 1s).'
    },
    'Kafka.FlushFrequency': {
      type: 'go-duration', label: 'Flush 주기', default: '500ms',
      description: '배치 flush 주기입니다. Go duration 형식 (예: 500ms, 1s).'
    },
    'Kafka.FlushMessages': {
      type: 'number', label: 'Flush 메시지 수', default: 100,
      description: 'flush 트리거 메시지 수입니다. 이 수만큼 쌓이면 즉시 전송합니다.'
    },
    'Kafka.BatchSize': {
      type: 'number', label: '배치 크기 (bytes)', default: 16384,
      description: '배치 최대 크기(bytes)입니다.'
    },
    'Kafka.Timeout': {
    ...
    VirtualAddressList: {
      ...
      description: 'IP 감지에 사용할 가상 주소 목록입니다. 쉼표로 구분합니다.'
    },
    ServiceDiscoveryPort: {
      type: 'number', label: 'ServiceDiscovery 포트', default: 50009,
      description: 'ServiceDiscovery HTTP 서비스 포트입니다.'
    },
    ...
    'SocksProxy.Host': {
      ...
      description: 'SOCKS5 프록시 호스트 주소입니다. 비어있으면 프록시를 사용하지 않습니다.'
    },
    'SocksProxy.Port': {
      type: 'number', label: 'SOCKS 포트', default: 0,
      description: 'SOCKS5 프록시 포트입니다.'
    },
    ...
    'Redis.Port': {
      ...
      description: 'Redis 포트 번호입니다. 호스트는 감지된 서버 IP를 사용합니다.'
    },
    'Redis.Password': {
  ...
  fieldGroups: [
    ...
    {
      name: '파일 출력',
      fields: ['File.FilePath', 'File.MaxSizeMB', 'File.MaxBackups', 'File.Console', 'File.Pretty', 'File.Format'],
      showWhen: { field: 'SenderType', value: 'file' }
    },
    {
      name: 'Kafka 설정',
      fields: [
        'Kafka.Brokers', 'Kafka.Compression', 'Kafka.RequiredAcks',
        'Kafka.MaxRetries', 'Kafka.RetryBackoff', 'Kafka.FlushFrequency', 'Kafka.FlushMessages', 'Kafka.BatchSize', 'Kafka.Timeout',
...
export function parseResourceAgentInput(config) {
  ...
  for (const [fieldName, fieldDef] of Object.entries(RESOURCEAGENT_SCHEMA.fields)) {
    ...
    if (dotIdx > 0) {
      // Nested field: 'Kafka.Topic' -> config.Kafka?.Topic
