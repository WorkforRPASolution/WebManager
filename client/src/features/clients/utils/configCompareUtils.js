/**
 * N-way Config Compare 순수 함수 유틸리티 (프론트엔드)
 */
...
export function computeDiff(baselineEqpId, clientFlatMaps, allKeys) {
  ...
  for (const key of allKeys) {
    ...
    if (baseEntry && (baseEntry.type === 'object' || baseEntry.type === 'array')) continue
    ...
    for (const eqpId of eqpIds) {
      const entry = clientFlatMaps[eqpId]?.get(key)
      if (!entry) {
        keyDiff.set(eqpId, { value: undefined, isDifferent: true, isMissing: true, type: undefined })
      } else if (eqpId === baselineEqpId) {
        keyDiff.set(eqpId, { value: entry.value, isDifferent: baselineMissing, isMissing: false, type: entry.type })
      } else {
        const currentStr = JSON.stringify(entry.value)
        const isDifferent = baselineMissing || baselineStr !== currentStr
        keyDiff.set(eqpId, { value: entry.value, isDifferent, isMissing: false, type: entry.type })
      }
    }
