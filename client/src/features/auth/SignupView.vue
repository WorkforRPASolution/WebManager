...
const isIntegrated = computed(() => operationMode.value === 'integrated')

// EARS selection state
const earsSelectedUser = ref(null) // { cn, employeeNumber, department, mail }
const hasEarsSelection = computed(() => !!earsSelectedUser.value)
...
const isFormValid = computed(() => {
  const base = form.value.name.trim().length >= 2 &&
  ...
  if (isIntegrated.value) {
    return base && hasEarsSelection.value
  }
  return base
})
...
// Reset ID check when singleid changes (skip if EARS selected)
watch(() => form.value.singleid, () => {
  if (!hasEarsSelection.value) {
    idChecked.value = null
