...
const { bookmarks, add: addBookmark, remove: removeBookmark } = useFilterBookmarks('popup-template')
...
const fetchProcesses = async () => {
  try {
    const response = await popupTemplateApi.getProcesses()
    // Popup Template uses POPUP_TEMPLATE data source
    processFilterStore.setProcesses('popupTemplate', response.data)
    processes.value = processFilterStore.getFilteredProcesses('popupTemplate')
  } catch (error) {
...
const fetchAllModels = async () => {
  try {
    ...
    const response = await popupTemplateApi.getModels(null, userProcesses)
...
const fetchAllCodes = async () => {
  try {
    ...
    const response = await popupTemplateApi.getCodes(null, null, userProcesses)
...
const fetchModelsForProcesses = async (processArray) => {
  ...
  try {
    const response = await popupTemplateApi.getModels(processArray.join(','))
...
const fetchCodesForFilters = async (processArray, modelArray) => {
  ...
  try {
    ...
    const response = await popupTemplateApi.getCodes(
      processArray?.join(',') || '',
      modelArray?.join(',') || '',
      userProcesses
    )
