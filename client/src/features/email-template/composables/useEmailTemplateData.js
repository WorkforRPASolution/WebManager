...
export function useEmailTemplateData() {
  ...
  const fetchData = async (filters = {}, page = 1, size = 25) => {
    ...
    error.value = null
    try {
      const response = await emailTemplateApi.getAll(filters, page, size)
  ...
  const save = async () => {
    ...
    try {
      ...
      if (deletedRows.value.size > 0) {
        ...
        const deleteRes = await emailTemplateApi.delete(deleteIds)
      ...
      if (rowsToCreate.length > 0) {
        ...
        const createRes = await emailTemplateApi.create(createData)
      ...
      if (rowsToUpdate.length > 0) {
        const updateRes = await emailTemplateApi.update(rowsToUpdate)
