...
| CRON_RUN_LOG | `initializeRecoverySummary()` | `server/features/recovery/indexManager.js` | unique + TTL(800일) 인덱스 생성 |
...
## CRON_RUN_LOG (배치 실행 이력)

Recovery Summary 배치(cron/backfill) 실행 이력. Gap 감지 및 backfill 트리거에 활용.

### Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| jobName | String | Required | 작업명 (`recoverySummary`) |
| period | String | Required | `hourly` 또는 `daily` |
| bucket | Date | Required | 시계열 버킷 시작 시각 (UTC) |
| status | String | Optional | `success`, `partial`, `failed`, `running` |
| startedAt | Date | Optional | 실행 시작 시각 |
| completedAt | Date | Optional | 실행 완료 시각 |
| docsMatched | Number | Optional | 원본(EQP_AUTO_RECOVERY) 건수. 빈 버킷(0) vs 데이터 유실(>0) 구분용. verify 옵션에서 사용 |
| pipelineResults | Map<String, String> | Optional | 파이프라인별 결과 (`{ scenario: 'success', equipment: 'failed: ...' }`) |
| errorMessage | Array<{configKey, error}> | Optional | 실패 파이프라인 에러 메시지 |
| source | String | Optional | 실행 출처 (`cron`, `autoBackfill`, `manualBackfill`) |

### Indexes

- `{ jobName: 1, period: 1, bucket: 1 }` (unique) — bucket별 멱등 upsert용
- `{ completedAt: 1 }` (TTL 800일 = `expireAfterSeconds: 69120000`) — 조회 최대 730일 + 70일 버퍼

### TTL 정책

CRON_RUN_LOG의 TTL은 조회 최대 범위(730일, custom 기간) 이상으로 설정.
800일 보관 시 데이터 규모: ~20,000건, ~6MB — 인덱스 성능 영향 무의미.
...
서버 시작 시 `initializeRecoverySummary()`에서 인덱스 자동 생성.
