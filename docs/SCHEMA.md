...
각 프로필은 독립된 name/osVer/version/packages/source를 가지며, OS별·버전별 다중 배포 구성을 지원합니다.
...
| profiles[].packages | Array | Optional | 배포 패키지 목록 |
| profiles[].packages[].packageId | String | Required | 패키지 ID (자동 생성: `pkg_` 접두어) |
| profiles[].packages[].name | String | Required | 패키지 표시명 (예: `Agent Binary`) |
| profiles[].packages[].targetPath | String | Required | basePath 기준 상대경로 (예: `bin/agent.jar`) |
| profiles[].packages[].targetType | String | Optional | `file` (단일 파일) 또는 `directory` (폴더 전체). 기본: `file` |
| profiles[].packages[].description | String | Optional | 패키지 설명 |
...
서버 시작 시 `initializeUpdateSettings()`가 레거시 문서(flat `packages[]` + `source`)를 자동 감지하여
`profiles: [{ profileId: 'prof_default', name: 'Default', ... }]` 형태로 변환합니다.
변환 후 기존 `packages`, `source` 필드는 `$unset`으로 제거됩니다.
...
3차: 사용자가 남은 프로필 중 수동 선택 → 패키지 선택 → Deploy
```
...
```javascript
{
  ...
  profiles: [
    ...
    },
    {
      ...
      version: "2.0.0",
      packages: [
        { packageId: "pkg_2", name: "Agent Binary", targetPath: "bin/agent.jar", targetType: "file" }
      ],
