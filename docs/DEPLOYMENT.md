...
| 항목 | 적용 시점 | 명령 | 가이드 |
|------|---------|------|--------|
| UPDATE_SETTINGS per-profile 정규화 | 구 shape(`profiles[]` 배열) → `(agentGroup, profileId)` per-doc | `npm run migrate:update-settings -- --yes` | [UPDATE_SETTINGS_SCHEMA_MIGRATION.md](./UPDATE_SETTINGS_SCHEMA_MIGRATION.md) |
...
**실패 시**: 부팅 가드 메시지가 `Run: npm run migrate:update-settings`를 직접 가리킵니다. 스크립트는 idempotent하므로 재실행 안전.
