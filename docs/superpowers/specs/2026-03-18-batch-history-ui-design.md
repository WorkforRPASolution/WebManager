...
- **권한**: Admin(role 1)만 접근 — 라우트에 `requireRole([1])` 적용
...
- **셀 색상**: 정상 실행 건수에 따라 초록 농도 (연한→진한), skip은 노란색, backfill은 파란색
- **색상 우선순위**: skip > backfill > cron (하루에 여러 유형이 있으면 가장 주의가 필요한 색)
- **셀 클릭**: 해당 날짜로 테이블 필터링 (startDate/endDate를 해당 날짜 KST 00:00~23:59로 설정)
- **Tooltip**: 날짜, 총 건수, 유형별 건수
- **레이아웃**: 요일(Mon/Wed/Fri/Sun) × 주(columns), 오른쪽이 최신
- **구현**: 순수 HTML div (ECharts 불필요)

### 히트맵 카테고리 매핑

6개 batchAction을 3개 히트맵 카테고리로 매핑:

| 히트맵 카테고리 | batchAction | 색상 |
|----------------|-------------|------|
| cron | `cron_completed` | 초록 (농도로 건수 표현) |
| skip | `cron_skipped` | 노란 |
| backfill | `backfill_started`, `backfill_completed`, `backfill_cancelled`, `auto_backfill_completed` | 파란 |

색상 결정 로직: 하루 중 skip이 1건이라도 있으면 노란색, backfill이 있으면 파란색, cron만 있으면 초록. 복합 시 우선순위: skip > backfill > cron.
...
- **총 건수**: 우측에 "총 N건" 표시
- **히트맵 셀 클릭 시**: 기간 드롭다운을 "직접" 표시로 변경하고, startDate/endDate를 해당 날짜로 설정. 드롭다운 선택 시 다시 상대 기간 모드로 복귀.
...
- 서버 사이드 페이지네이션 (기존 `parsePaginationParams` + `createPaginatedResponse` 사용)
- 페이지당 50건, 하단에 페이지 네비게이션

### UI 상태

- **로딩**: 테이블 영역에 스피너 표시
- **에러**: "데이터를 불러오지 못했습니다" 메시지 + 재시도 버튼
- **빈 상태**: "조건에 맞는 이력이 없습니다" 메시지

## API

### GET /api/recovery/batch-logs

배치 이력 목록 조회. controller에서 `WebManagerLog.countDocuments()` + `WebManagerLog.find()` 쿼리 실행.

**Query Parameters:**
- `batchAction` (optional): 필터
- `period` (optional): today / 7d / 30d — 상대 기간
- `startDate`, `endDate` (optional): 절대 기간 (히트맵 셀 클릭 시)
- `page`, `pageSize` (optional): 페이지네이션 (기본 page=1, pageSize=50)

**Response** (기존 `createPaginatedResponse` 형식):
```json
{
  "data": [...],
  "pagination": {
...
히트맵 데이터 전용 API. MongoDB aggregation으로 일별 집계.
...
```javascript
// $match: category='batch', timestamp >= now - days
// $group: _id = { date(KST), batchAction }, count
// $group: _id = date, 카테고리별 합산
```
...
히트맵 카테고리 매핑 (서버에서 처리):
- `cron`: `cron_completed` 건수
- `skip`: `cron_skipped` 건수
- `backfill`: `backfill_started` + `backfill_completed` + `backfill_cancelled` + `auto_backfill_completed` 건수

### 권한

- 라우트에 `requireRole([1])` 적용 (Admin 전용)
- 프론트엔드: userStore.role === 1일 때만 버튼 표시
...
| `server/features/recovery/routes.js` | 2개 라우트 추가 (`requireRole([1])`) |
...
`server/features/recovery/controller.batchLogs.test.js`:

1. **getBatchLogs**: 기본 조회 — pagination 형식 검증
2. **getBatchLogs**: batchAction 필터 — 특정 유형만 반환
3. **getBatchLogs**: period 필터 — today/7d/30d 날짜 범위 변환 검증
4. **getBatchLogs**: startDate/endDate 절대 기간 — 히트맵 셀 클릭 시나리오
5. **getBatchHeatmap**: 기본 30일 — 일별 집계 응답 형식 검증
6. **getBatchHeatmap**: days=90 — 기간 파라미터 전달 검증
7. **getBatchHeatmap**: 카테고리 매핑 — cron/skip/backfill 분류 검증
