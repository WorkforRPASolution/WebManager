...
| 파일 | 역할 | 위치 |
|------|------|------|
| **ResourceAgent.json** | 에이전트 식별, 전송 방식, Kafka/Redis 연결, 네트워크 설정 | `conf/ResourceAgent/` |
...
**핵심 동작 흐름**: ResourceAgent.json에서 에이전트 식별 및 전송 설정 로드 → Monitor.json에서 Collector별 수집 설정 로드 → Scheduler가 주기적으로 Collector 실행 → Sender가 메트릭 전송
...
- **중첩 구조**: 섹션별 Object로 그룹화 (`Agent`, `Kafka`, `Redis`, `File`, `SocksProxy`)
...
| 타입 | `string` |
| 기본값 | `""` (빈 문자열) |
| 설명 | 에이전트 고유 ID. 비어있으면 Redis EQP_INFO의 `EqpID` → 시스템 hostname 순으로 대체 |
| JSON 키 | `Agent.ID` |
| 비고 | 운영 환경에서는 보통 비워두고 EQP_INFO에서 자동 결정 |
...
| 타입 | `string` |
| 기본값 | `""` (빈 문자열) |
| 설명 | 호스트명 오버라이드. 비어있으면 `os.Hostname()` 결과 사용 |
| JSON 키 | `Agent.Hostname` |

#### `Agent.Tags`
| 속성 | 값 |
|------|-----|
| 타입 | `object` (`map[string]string`) |
| 기본값 | `{}` (빈 맵) |
| 설명 | 사용자 정의 태그. 메트릭 전송 시 메타데이터로 포함 |
| JSON 키 | `Agent.Tags` |
| 예시 | `{"environment": "production", "location": "factory-1"}` |
| 비고 | key-value 쌍 자유 입력. Merge 시 키 단위로 병합 |
...
| 값 | 동작 |
|-----|------|
| `"json"` | MetricData 구조체를 JSON으로 직접 기록 |
| `"legacy"` | EARS 평문 호환 포맷으로 기록 |

### 3.5 Kafka 섹션

`SenderType`이 `"kafka"`일 때 사용하는 Kafka 연결 설정입니다. 18개 필드로 구성됩니다.
...
| 타입 | `string` |
| 기본값 | `"factory-metrics"` |
| 설명 | Kafka 토픽명. 런타임 시 `ResourceMonitorTopic` 모드에 의해 재결정될 수 있음 |
| JSON 키 | `Kafka.Topic` |
...
| JSON 키 | `Kafka.MaxRetries` |

#### `Kafka.RetryBackoff` (duration)
| 속성 | 값 |
|------|-----|
| 타입 | `string` (duration) |
| 기본값 | `"100ms"` |
| 설명 | 재시도 간 대기 시간 |
| JSON 키 | `Kafka.RetryBackoff` |

#### `Kafka.FlushFrequency` (duration)
| 속성 | 값 |
|------|-----|
| 타입 | `string` (duration) |
| 기본값 | `"500ms"` |
| 설명 | 배치 flush 주기 |
| JSON 키 | `Kafka.FlushFrequency` |
...
| 타입 | `number` (정수) |
| 기본값 | `100` |
| 설명 | flush 트리거 메시지 수. 이 수만큼 쌓이면 즉시 전송 |
| JSON 키 | `Kafka.FlushMessages` |
...
| 설명 | Kafka 토픽 명명 모드. EQP_INFO 기반으로 토픽명 자동 생성 |
