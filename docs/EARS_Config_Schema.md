...
Akka `QuartzSchedulerExtension`이 사용하는 Quartz cron 표현식. **Unix cron(5필드)과 달리 6~7필드**이며 초 필드가 맨 앞에 추가됩니다.
...
→ 매 분 정각(`:00`초)마다 1회 발화 (시간당 60회, 일당 1440회).

**`?`의 의미**: `?`는 "no specific value". 일(day-of-month)과 요일(day-of-week)은 동시에 구체적으로 지정할 수 없으므로 한쪽을 지정하면 다른 쪽은 `?`로 비웁니다.

| 표현 | 의미 |
|------|------|
| `"0 * * * * ?"` | 일=`*`이므로 요일=`?` |
| `"0 0 9 ? * MON"` | 매주 월요일 09:00:00 (요일 지정, 일은 `?`) |
| `"0 0 0 1 * ?"` | 매월 1일 00:00:00 (일 지정, 요일은 `?`) |
