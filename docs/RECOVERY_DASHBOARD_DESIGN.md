...
### ⚠️ 시간대(Timezone) 주의사항 — 저장은 UTC, 표시는 KST

MongoDB `Date` 타입은 **내부적으로 항상 UTC 밀리초**로 저장된다.
KST(+09:00) 오프셋은 입력 시 파싱되어 UTC로 변환된다.

**예시: KST 2026-03-18 자정 → MongoDB 저장값**
```
입력: ISODate("2026-03-18T00:00:00.000+09:00")
저장: ISODate("2026-03-17T15:00:00.000Z")    ← mongosh에서 보이는 값
```

**이 패턴을 사용하는 이유:**

1. **MongoDB 제약**: `Date` 타입 자체가 timezone 정보를 저장하지 않음. KST 값을 UTC인 척 저장하면(예: `2026-03-18T00:00:00.000Z`) aggregation 연산자(`$dateToString`, `$dateTrunc` 등)가 **9시간 어긋난 결과**를 반환
2. **$merge 멱등성**: `bucket` 필드가 `$merge.on` 키로 사용되어 같은 bucket을 재처리해도 중복 없이 upsert됨. UTC 기준으로 통일해야 `dateUtils.js`의 경계 계산과 정확히 일치
3. **Aggregation 호환**: `$dateToString: { timezone: '+09:00' }` 옵션으로 조회 시 KST 변환이 정확하게 동작

**mongosh에서 KST 확인하는 방법:**
```javascript
// bucket을 KST로 확인
db.RECOVERY_SUMMARY_BY_SCENARIO.find().limit(3).forEach(d => {
  print('UTC:', d.bucket.toISOString(), '→ KST:', d.bucket.toLocaleString('ko-KR', {timeZone:'Asia/Seoul'}), '| process:', d.process)
})

// 특정 KST 날짜 범위 조회 (KST 03/18 하루)
db.RECOVERY_SUMMARY_BY_SCENARIO.find({
  bucket: {
    $gte: ISODate("2026-03-17T15:00:00.000Z"),  // KST 03/18 00:00
    $lt:  ISODate("2026-03-18T15:00:00.000Z")   // KST 03/19 00:00
  }
}).count()
```

**코드에서의 KST 경계 계산 (`dateUtils.js`):**
```
KST 자정 = UTC 전일 15:00
KST 03/18 00:00 ~ 03/19 00:00  =  UTC 03/17 15:00 ~ 03/18 15:00
```

> **결론**: DB에 저장된 날짜가 "하루 전"으로 보이는 것은 정상이다.
> 예를 들어 `bucket: 2026-03-17T15:00:00.000Z`는 **KST 2026-03-18 자정** bucket이다.
...
|---------|------------------------------------|-----------------------------------|
| hourly  | 매 시 :05분                        | settling 적용 후 직전 완성된 1시간 |
| daily   | 매일 `SETTLING_HOURS`:10 KST (동적) | settling 적용 후 전날 00:00~24:00 (KST) |

> **settling 시간**: `RECOVERY_SETTLING_HOURS` 환경변수 (기본 3시간).
> hourly/daily 모두 동일하게 `now - settlingHours`로 기준 시각을 계산한 뒤 bucket을 결정한다.
> 예: settling=3이면 daily cron은 03:10 KST에 실행되어 어제 데이터를 집계한다.
> 이렇게 하면 늦게 도착하는 데이터(최대 settling 시간 이내)도 누락 없이 집계된다.
...
[EQP_AUTO_RECOVERY 원본 ~1,500만 건 (EARS DB, Replica Set)]
      ...
      │                                         │
      │  node-cron                              │  API 직접 조회
      │  (매 :05분 / 매일 SETTLING:10)           │  (단일 eqpid/ears_code, 최대 7일)
      │  ┌──────────────────────┐               │
      │  │ 동시 실행 방지       │               │  인덱스:
