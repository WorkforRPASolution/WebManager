...
| `PUT` | `/api/clients/:id/config/:fileId` | 단일 Config 파일 저장 (FTP, 자동 백업) | 60초 |
| `POST` | `/api/clients/config/deploy` | 횡전개 실행 (SSE 진행률, 자동 백업) | SSE |
| `GET` | `/api/clients/:id/config/:fileId/backups` | Config 백업 목록 조회 | 30초 |
| `GET` | `/api/clients/:id/config/:fileId/backups/:backupName` | Config 백업 내용 조회 | 30초 |
...
**GET /:id/config/:fileId/backups**
```json
[
  {
    "name": "20260223_153042.json",
    "timestamp": "20260223_153042",
    "date": "2026-02-23T06:30:42.000Z",
    "size": 1234
  }
]
```

**GET /:id/config/:fileId/backups/:backupName**
```json
{
  "content": "{ ... }"
}
...
└── features/clients/
    ...
    ├── configController.js      # Config 핸들러 (settings, read, write, deploy, backup)
    ├── routes.js                # +7 Config 라우트
...
└── features/clients/
    ...
    ├── composables/
    │   └── useConfigManager.js  # Config 상태 관리 composable (+백업 로드/복원)
    └── components/
        ├── ConfigManagerModal.vue    # Config 관리 풀스크린 모달
        ├── ConfigBackupDropdown.vue  # 백업 목록 드롭다운
...
│                              [Diff] [Discard] [Backup] [Save] [Deploy] │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
...
Config 파일 저장/횡전개 시 기존 파일을 자동 백업하고, 백업 목록에서 이전 버전을 복원할 수 있는 기능.

| 항목 | 내용 |
|------|------|
| 백업 시점 | 단건 저장 + 횡전개 시 (덮어쓰기 전 자동) |
| 저장 위치 | 원격 장비 FTP, `{parentDir}/backup/{configFileName}/` |
| 파일명 형식 | `YYYYMMDD_HHmmss.{원본확장자}` |
| 최대 보관 | 각 Config 파일당 5개 (초과 시 가장 오래된 것 자동 삭제) |
| 복원 방식 | 백업 내용을 에디터에 로드 → 사용자가 확인/수정 후 Save |
| DB 사용 | 없음 (순수 FTP 파일 기반) |

### 백업 디렉토리 구조

```
{configPath의 parentDir}/backup/{configFileName}/
  ├── 20260223_153042.json
  ├── 20260222_091530.json
  └── ...                       # 최대 5개
```

예: `/app/conf/ARSAgent/application.json`의 백업
→ `/app/conf/ARSAgent/backup/application.json/20260223_153042.json`

### UI

Config Manager Modal 툴바의 Discard와 Save 버튼 사이에 **Backup** 드롭다운 배치:
- 버튼 클릭 → 드롭다운 패널 + 백업 목록 로드
- 항목 클릭 → 에디터에 백업 내용 로드 (저장은 사용자가 직접 Save)
- 외부 클릭 시 닫기

### 보안

`backupName` 파라미터에 path traversal 방지를 위한 형식 검증 적용:
- 허용 패턴: `/^\d{8}_\d{6}\.\w+$/` 또는 `/^\d{8}_\d{6}$/`
