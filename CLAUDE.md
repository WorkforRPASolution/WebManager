...
| Dashboard - ARSAgent Monitor | Process별 ARSAgent 가동 현황 (Running/Stopped/NeverStarted 3상태) |
...
MainMenu: Dashboard
├── SubMenu: Overview (/)
└── SubMenu: ARSAgent Monitor (/agent-monitor)
...
- ARSAgent Monitor 완료 (Process별 가동 현황 + 도넛/바 차트 + 3상태 분류(Running/Stopped/NeverStarted) + AgentMetaInfo 활용 + Process/Model 필터)
- Dashboard 서브메뉴별 권한 분리 (dashboard → dashboardOverview + dashboardArsMonitor, 향후 확장 용이)
